...
struct PM : public vector<Z>{  // DO NOT USE POINTER !!
	...
	static constexpr unsigned int mo=Z::umod();
	template<class>static constexpr bool failed_mod=false;
	static constexpr Z g=[]()->Z {
		if constexpr (mo==167772161 || mo==469762049 || mo==998244353 || mo==1004535809){
			return Z::raw(3);
		}
		else if constexpr (mo==(int)(1e9+7)) return Z::raw(5);
		else if constexpr (mo==754974721) return Z::raw(11);
		else{
			static_assert(failed_mod<Z>,"No primitive root found for this modulus");
			return -1;
		}
	}();
	static constexpr Z gi=g.inv();
...
// using Z=MI<TODO>; // contained in Mod-Int-Static
// #define raw(x) Z::raw(x) // contained in Mod-Int-Static
using Poly=PM<Z>;