...
\newfontfamily\LocalMono{MapleMonoNormalNL}[
	Path = ./MapleMonoNormalNL-TTF/,
	Extension = .ttf,  % 或 .otf
	UprightFont = *-Regular,
	BoldFont = *-Bold,
	ItalicFont = *-Italic,
...
\lstdefinestyle{lfonts}{
	basicstyle   = \footnotesize\LocalMono,
	stringstyle  = \color{purple}\LocalMono,
	keywordstyle = \color{blue!60!black}\bfseries\LocalMono,
	commentstyle = \color{olive}\LocalMono,
	identifierstyle = \LocalMono,
	numberstyle = \tiny\LocalMono
}
...
\lstdefinestyle{lgeneral}{
	...
	literate = {*}{\raisebox{-0.13ex}{*}}1,  % 调整*的位置
}
\lstdefinestyle{C++}{
	language = {C++},
	style    = lgeneral,
	morekeywords = {constexpr, consteval},  % 添加 constexpr 到关键字列表
	% morecomment = [l][\color{green!50!black}\bfseries]{\#},  % # 开头的用绿色高亮
}
...
\subsubsection{Dynamic Xor Sum Set}

e.g. \href{https://codeforces.com/gym/105813/problem/D}{GYM105813D}. \texttt{xorsum<S,K>}, \texttt{S} 是整型, \texttt{K} 是位数

\begin{enumerate}
	\item \textbf{modi(x)} If $x$ is currently in the set $S$, remove it. Otherwise, add $x$ to the set.
	\item \textbf{query(t)} Return $\displaystyle\bigoplus_{x \in S} (x + t)$.
\end{enumerate}

\subsubsubsection{rb\_{}tree 写法 (常数大)}

\lstinputlisting[style=C++]{"../Template/Data Structures/Dynamic Xor Sum Set (rb_tree).h"}

\newpage

\subsubsubsection{Fenwick 写法 (常数小)}

\lstinputlisting[style=C++]{"../Template/Data Structures/Dynamic Xor Sum Set (Fenwick).h"}
...
\subsubsection{可持久化文艺平衡树}

模板题是 \href{https://codeforces.com/contest/2242/problem/F}{CF2242F}

\lstinputlisting[style=C++]{../Template/Data Structures/Persistent fhqTreap Wenyi.h}
...
\lstinputlisting[style=C++]{../Template/Data Structures/Ring Buffer.h}
...
\subsection{二分图最大匹配（bitset 优化 KM）}

注意点的下标从 0 开始

\lstinputlisting[style=C++]{../Template/Graphs/Kuhn–Munkres (bitset).h}
...
\lstinputlisting[style=C++]{../Template/Math/FWT XOR.h}

\newpage

\subsection{(扩展) 中国剩余定理}

同时包含了 CRT 和 exCRT

\lstinputlisting[style=C++]{../Template/Math/Chinese Remainder Theorem.h}

\subsection{Lucas 定理}

\lstinputlisting[style=C++]{../Template/Math/Lucas Theorem.h}

\newpage

\subsection{扩展 Lucas 定理}

\lstinputlisting[style=C++]{../Template/Math/Lucas Theorem Ex.h}
...
\subsection{std::views 语法糖}

\input{"../Template/Other/std_ranges_views.tex"}
...
\subsection{某个数论 trick}

对于两个整数 $u, v \in [0, m-1]$，(\href{https://qoj.ac/contest/2609/problem/14719}{2025 ICPC 武汉 A})

$$
[u < v] = [u \leq v-1] = 1 + \left\lfloor \frac{(v-1) - u}{m} \right\rfloor
$$
